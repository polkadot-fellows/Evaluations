...
- [#12773](https://github.com/paritytech/polkadot-sdk/pull/12773) (open): wraps every `StakingLedger` mutation in `pallet-staking-async` (`update`, `bond`, `set_payee`, `set_controller_to_stash` and `kill`) in a transactional storage layer that rolls back atomically if the post-mutation consistency check fails, closing the class of state corruption described in [#3245](https://github.com/paritytech/polkadot-sdk/issues/3245) and [#3697](https://github.com/paritytech/polkadot-sdk/issues/3697).
- [#12899](https://github.com/paritytech/polkadot-sdk/pull/12899) (open): replaces `pallet-xcm`'s deprecated `LockableCurrency` with the `fungible` freeze traits, so locally locked assets are held under a new `FreezeReason::AssetLock` rather than the `py/xcmlk` lock, with a multi-block migration and a lazy fallback for pre-existing locks. Part of [#7226](https://github.com/paritytech/polkadot-sdk/issues/7226).
