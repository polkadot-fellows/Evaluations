...
Continued deferred dispatch for `pallet-whitelist`: a referendum-enacted whitelisted call is banked in storage instead of consuming the authorization when enactment races ahead of whitelisting. This period addressed review feedback and integration testing.
...
Under the [hosting pilot](https://hackmd.io/529HcCT2TCGYQVRIYoeDzg), we resumed OpenDev calls on [2026/06/30](https://forum.polkadot.network/t/opendev-call-proceedings-30-june-2026/18029), which centred on RFC #171 (asset-based storage deposits) and settled on migrating pallets off hardcoded `Currency` deposits onto `Consideration` as a priority.
